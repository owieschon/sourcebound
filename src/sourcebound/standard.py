...
def _exemplar_asset(standard: Path) -> tuple[str, str]:
    local = standard.parent / "src/sourcebound/standards/exemplars.md"
    if local.exists():
        text = local.read_text(encoding="utf-8")
    else:
        resource = resources.files("sourcebound").joinpath("standards/exemplars.md")
...
def load_default_pack() -> dict[str, Any]:
    resource = resources.files("sourcebound").joinpath("standards/default.json")
    with resources.as_file(resource) as path:
